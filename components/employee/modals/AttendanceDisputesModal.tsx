...
export default function AttendanceDisputesModal({ open, onClose, cancelDispute, cancelingDisputeId, disputeClaimed, disputeFieldLabel, disputeOriginal, disputeTypeLabel, formatDisputeTimePh, myDisputes, openDisputeModal, selectedMyDisputeDetail, setSelectedMyDisputeDetail }: Props) {
  ...
  return (
    <ModalShell open={open} onClose={close} title={localize(selectedMyDisputeDetail ? 'Dispute Details' : 'My Disputes')} size="sm" closeDisabled={Boolean(cancelingDisputeId)}>
            ...
            {!selectedMyDisputeDetail && (
              ...
              >
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg><T>{" Report Missing Log "}</T></button>
            )}
              ...
              {selectedMyDisputeDetail ? (
                <div className="space-y-3">
                  ...
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-2">
                    <div>
                      <p className="label-branded mb-0.5"><T>{"Dispute Date"}</T></p>
                      <p className="text-slate-700 text-xs">{selectedMyDisputeDetail.dispute_date}</p>
                    </div>
                    {disputeOriginal(selectedMyDisputeDetail) && (
                      <div>
                        <p className="label-branded mb-0.5"><T>{"Original "}</T>{disputeFieldLabel(selectedMyDisputeDetail)}</p>
                        <p className="text-slate-700 text-xs">{formatDisputeTimePh(disputeOriginal(selectedMyDisputeDetail)!)}</p>
                      </div>
                    )}
                    <div>
                      <p className="label-branded mb-0.5"><T>{"Claimed "}</T>{disputeFieldLabel(selectedMyDisputeDetail)}</p>
                  ...
                  <div className="text-slate-400 text-[10px] pt-1">
                    {selectedMyDisputeDetail.reviewed_at && (
                      <p><T>{"Resolved: "}</T>{new Date(selectedMyDisputeDetail.reviewed_at).toLocaleString('en-US', { timeZone: 'Asia/Riyadh', month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                    )}
                    <p><T>{"Filed: "}</T>{new Date(selectedMyDisputeDetail.created_at).toLocaleString('en-US', { timeZone: 'Asia/Riyadh', month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                  ...
                  {selectedMyDisputeDetail.status === 'Pending' && (
                    ...
                    >
                      <T>{cancelingDisputeId === selectedMyDisputeDetail.id ? 'Cancelling...' : 'Cancel This Dispute'}</T>
                    </button>
                  )}
                </div>
              ) : myDisputes.length === 0 ? (
                <div className="text-center py-10 border-2 border-dashed border-slate-200 rounded-2xl">
                  ...
                  <p className="text-slate-400 text-sm font-medium"><T>{"No disputes yet"}</T></p>
                </div>
              ) : (
            ...
            >
              <T>{selectedMyDisputeDetail ? '← Back' : 'Close'}</T>
            </button>
    </ModalShell>
  );
}
