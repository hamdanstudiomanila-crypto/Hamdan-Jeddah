...
export default function HelpDeskRequestsModal({ open, onClose, loading, requests, drafts, setDrafts, savingId, onSave }: Props) {
  ...
  return <ModalShell open={open} onClose={onClose} closeDisabled={Boolean(savingId)} title={localize("Help Desk / HR Requests")} description={localize("Review employee concerns, update status, and send an HR response")} icon="🎫" size="lg">
    {loading ? <LoadingRow label={localize("Loading employee requests...")} /> : requests.length === 0 ? <EmptyState icon="🎫" title={localize("No Help Desk requests yet")} description={localize("New employee concerns will appear here.")} /> : <div className="space-y-3">{requests.map((request) => {
      ...
      return <article key={request.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800">
        ...
        <div className="mt-3 grid grid-cols-1 items-end gap-2 sm:grid-cols-[150px_1fr_auto]"><div><label className="label-branded"><T>{"Status"}</T></label><select className="input-field min-h-11 !py-2 !text-xs" value={draft.status} onChange={(event) => setDrafts((current) => ({ ...current, [request.id]: { ...draft, status: event.target.value } }))}><option value="Submitted"><T>{"Submitted"}</T></option><option value="In Progress"><T>{"In Progress"}</T></option><option value="Resolved"><T>{"Resolved"}</T></option></select></div><div><label className="label-branded"><T>{"HR Response"}</T></label><input className="input-field min-h-11 !py-2 !text-xs" value={draft.hr_notes} placeholder="Response shown to employee..." onChange={(event) => setDrafts((current) => ({ ...current, [request.id]: { ...draft, hr_notes: event.target.value } }))}/></div><button type="button" onClick={() => onSave(request.id)} disabled={savingId === request.id} className="btn-primary min-h-11 !px-4 !py-2.5 text-xs disabled:opacity-50"><T>{savingId === request.id ? 'Saving...' : 'Save response'}</T></button></div>
      </article>;
    })}</div>}
  </ModalShell>;
}
