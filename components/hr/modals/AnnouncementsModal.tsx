...
export default function AnnouncementsModal({ open, onClose, announcementContent, announcementId, announcementImageInputRef, announcementImagePreview, announcementImageUrl, announcementLoading, announcementMsg, announcementRemoveImage, announcementSaving, announcementUpdatedAt, clearAnnouncementImage, handleAnnouncementImageChange, publishAnnouncement, setAnnouncementContent }: Props) {
  ...
  return (
    <ModalShell open={open} onClose={onClose} title={localize("Announcements")} description={announcementUpdatedAt ? `Last: ${new Date(announcementUpdatedAt).toLocaleString('en-US', { timeZone: 'Asia/Riyadh', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}` : undefined} icon={<Megaphone size={17} strokeWidth={2.4}/>} size="lg" closeDisabled={announcementSaving}>
          <div className="overflow-y-auto flex-1 pe-1">
          ...
          {announcementLoading ? <LoadingRow label={localize("Loading...")} /> : (
            <>
              <textarea className="input-field w-full min-h-[80px] resize-y text-sm" placeholder={localize("Type the announcement that all employees will see...")} value={announcementContent} onChange={(e) => setAnnouncementContent(e.target.value)} />
                ...
                {(announcementImagePreview || (announcementImageUrl && !announcementRemoveImage)) ? (
                  <div className="relative inline-block">
                    ...
                    <button
                      ...
                      className="absolute -top-2 -end-2 w-6 h-6 rounded-full bg-white shadow border border-slate-200 flex items-center justify-center text-slate-500 hover:text-red-600 transition"
                      aria-label={localize("Remove image")}
                    >
                  ...
                  <label className="inline-flex items-center gap-1.5 text-blue-600 text-xs font-bold cursor-pointer hover:underline">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="9" cy="9" r="2"/><path d="M21 15l-5-5L5 21"/></svg><T>{" Add Photo (optional) "}</T><input
              ...
              <button onClick={publishAnnouncement} disabled={announcementSaving || !announcementContent.trim()} className="btn-primary mt-3 !py-2.5 !text-xs disabled:opacity-50">
                <T>{announcementSaving ? <span className="flex items-center justify-center gap-2"><Spinner size="sm"/><T>{"Publishing..."}</T></span> : announcementId ? 'Update Announcement' : 'Publish Announcement'}</T>
              </button>
            </>
          )}
          </div>
