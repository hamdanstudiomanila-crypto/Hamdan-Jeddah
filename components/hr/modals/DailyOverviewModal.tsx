...
export default function DailyOverviewModal({ modal, meta, records, initials, setModal }: Props) {
  ...
  return <ModalShell open onClose={() => setModal(null)} title={meta.title} description={meta.description} size="sm">
    ...
    <button type="button" onClick={() => setModal(null)} className="mt-4 w-full rounded-full bg-slate-100 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-100 dark:hover:bg-slate-700"><T>{"Close"}</T></button>
  </ModalShell>;
}
