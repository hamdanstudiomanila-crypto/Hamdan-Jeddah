...
export default function HolidaysModal({ open, onClose, addHoliday, deleteHoliday, holidayMsg, holidaySaving, holidays, holidaysLoading, newHolidayDate, newHolidayName, setNewHolidayDate, setNewHolidayName }: Props) {
  ...
  return (
    <ModalShell open={open} onClose={onClose} title={localize("Holidays")} description={localize("Dates employees won't be auto-marked Absent")} icon={<CalendarRange size={17} strokeWidth={2.4}/>} size="lg" closeDisabled={holidaySaving}>
          <div className="overflow-y-auto flex-1 pe-1">
            ...
            <div className="flex flex-col sm:flex-row gap-2 mb-4">
              ...
              <input
                type="text"
                placeholder={localize("Holiday name (e.g. Independence Day)")}
              ...
              >
                <T>{holidaySaving ? <span className="flex items-center justify-center gap-2"><Spinner size="sm" /><T>{"Adding..."}</T></span> : '+ Add Holiday'}</T>
              </button>
            </div>

            <div className="space-y-1.5 min-h-[80px]">
              {holidaysLoading && <LoadingRow label={localize("Loading holidays...")} />}
              {!holidaysLoading && holidays.length === 0 && (
                <p className="text-slate-400 text-xs"><T>{"No holidays added yet."}</T></p>
              )}
              {!holidaysLoading && holidays.map((h) => (
                <div key={h.id} className="flex items-center justify-between gap-2 p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                  ...
                  <button onClick={() => deleteHoliday(h.id)} className="text-rose-500 hover:text-rose-700 text-xs font-bold flex-shrink-0"><T>{"Remove"}</T></button>
                </div>
              ))}
            </div>
          </div>
