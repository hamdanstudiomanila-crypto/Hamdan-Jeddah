...
export default function ExportReportsModal({ employees, exportEmployeeId, setExportEmployeeId, open, onClose, availableCutoffs, exportCutoff, exportEmployeeMasterListCSV, exportEmployeeMasterListPDF, exportMsg, exportPayrollSummaryCSV, exportPayrollSummaryPDF, exportRawAttendanceCSV, exportRawAttendancePDF, exportingType, formatCutoffLabel, rawExportMonth, rawExportPeriod, rawExportPreviewCount, setExportCutoff, setExportMsg, setRawExportMonth, setRawExportPeriod }: Props) {
  ...
  return (
    <ModalShell open={open} onClose={onClose} title={localize("Export Reports")} icon={<FileDown size={17} strokeWidth={2.4}/>} size="sm" closeDisabled={Boolean(exportingType)} className="dark:!border-[#34453a] dark:!bg-[#151d18]">
            {exportMsg && (
            ...
            <div className="mb-4">
              <label htmlFor="attendance-export-employee" className="mb-1 block text-xs font-bold text-slate-700 dark:text-slate-200"><T>{"Employee for attendance reports"}</T></label>
              <select id="attendance-export-employee" className="input-field !text-xs" value={exportEmployeeId} disabled={!!exportingType} onChange={event => { setExportEmployeeId(event.target.value); setExportMsg(null); }}>
                <option value=""><T>{"All employees"}</T></option>
              ...
              <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-300"><T>{"Applies to Payroll Summary and Raw Attendance Log. Employee Master List includes all employees."}</T></p>
            </div>
            ...
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 mb-3 dark:!border-[#34453a] dark:!bg-[#0d1510]">
              <p className="font-bold text-slate-900 text-xs mb-1 dark:!text-white"><T>{"Payroll Summary"}</T></p>
              <p className="text-slate-500 text-[11px] mb-3 dark:!text-[#c4d0c7]"><T>{"Present/Late/Absent/Leave day counts and late minutes per employee, for one cutoff period."}</T></p>
              <select
                className="input-field !py-1.5 !text-xs !min-h-0 mb-2"
                value={exportCutoff}
                onChange={(e) => setExportCutoff(e.target.value)}
              >
                <option value=""><T>{"Select cutoff period..."}</T></option>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <T>{exportingType === 'payroll-csv' ? 'Exporting...' : 'Download CSV'}</T>
                </button>
                <button
                  type="button"
                ...
                >
                  <T>{exportingType === 'payroll-pdf' ? 'Preparing...' : 'Download PDF'}</T>
                </button>
              </div>
            </div>
            ...
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 mb-3 dark:!border-[#34453a] dark:!bg-[#0d1510]">
              <p className="font-bold text-slate-900 text-xs mb-1 dark:!text-white"><T>{"Employee Master List"}</T></p>
              <p className="text-slate-500 text-[11px] mb-3 dark:!text-[#c4d0c7]"><T>{"Name, designation, employment status, and hired date for every employee."}</T></p>
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  ...
                  className="w-full bg-slate-900 text-white text-xs font-bold py-2.5 rounded-full hover:bg-slate-700 transition disabled:bg-slate-300 disabled:text-slate-500 disabled:opacity-100 dark:!bg-[#243229] dark:!text-white dark:hover:!bg-[#304237] dark:disabled:!bg-[#344039] dark:disabled:!text-[#c4d0c7]"
                >
                  <T>{exportingType === 'master-csv' ? 'Exporting...' : 'Download CSV'}</T>
                </button>
                <button
                  type="button"
                  ...
                  className="w-full bg-red-600 text-white text-xs font-bold py-2.5 rounded-full hover:bg-red-700 transition disabled:opacity-50"
                >
                  <T>{exportingType === 'master-pdf' ? 'Preparing...' : 'Download PDF'}</T>
                </button>
              </div>
            </div>
            ...
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 dark:!border-[#34453a] dark:!bg-[#0d1510]">
              <p className="font-bold text-slate-900 text-xs mb-1 dark:!text-white"><T>{"Raw Attendance Log"}</T></p>
              <p className="text-slate-500 text-[11px] mb-3 dark:!text-[#c4d0c7]"><T>{" Export every attendance record for a whole month or one payroll cutoff. This export is independent from the collapsed Attendance History view. "}</T></p>

              <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1 dark:!text-[#c4d0c7]"><T>{"Month"}</T></label>
              <input
                type="month"
                className="input-field !py-1.5 !text-xs !min-h-0 mb-2"
              ...
              <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1 dark:!text-[#c4d0c7]"><T>{"Coverage"}</T></label>
              ...
              <p className={`text-[10px] font-bold mb-3 ${rawExportPreviewCount > 0 ? 'text-emerald-600 dark:text-emerald-300' : 'text-slate-500 dark:text-slate-300'}`}>
                {rawExportPreviewCount}<T>{" loaded matching record"}</T><T>{rawExportPreviewCount === 1 ? '' : 's'}</T><T>{" · complete period will be checked before export "}</T></p>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="w-full bg-slate-900 text-white text-xs font-bold py-2.5 rounded-full hover:bg-slate-700 transition disabled:bg-slate-300 disabled:text-slate-500 disabled:opacity-100 dark:!bg-[#243229] dark:!text-white dark:hover:!bg-[#304237] dark:disabled:!bg-[#344039] dark:disabled:!text-[#c4d0c7]"
                >
                  <T>{exportingType === 'raw-csv' ? 'Exporting...' : 'Download CSV'}</T>
                </button>
                <button
                  type="button"
                  ...
                  className="w-full bg-red-600 text-white text-xs font-bold py-2.5 rounded-full hover:bg-red-700 transition disabled:opacity-50"
                >
                  <T>{exportingType === 'raw-pdf' ? 'Preparing...' : 'Download PDF'}</T>
                </button>
              </div>
            </div>
